...
def assign_data_ip():
    ...
    for iface_name, iface in network_ifaces.items():
        logging.info(f"iface_name: {iface_name}, iface: {str(iface)}")
    ...
    for iface in access_interfaces:
        if iface.link_detected and iface.ipv4 and is_access_network_ip(iface.ipv4_address):
            logging.info(f"detected access iface {iface.name} which has ip {iface.ipv4_address}")
            access_ip_address = iface.ipv4_address
